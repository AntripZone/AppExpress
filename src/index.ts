...
app.get(
  ...
  function (req: Request<{}, {}, {}, estudiantesFiltrados>, res: Response) {
    const { activo, nombre, pais, minEdad, promMin } = req.query;
    ...
    if(promMin){
      const promedio =  Number (promMin);
      if(isNaN(promedio)){
        return res.status(404).json({error: "El promedio debe ser un numero"});
      }
      resultado = resultado.filter(e => calcularPromedio(e.notas) >= promedio);
    }
...
/* Eliminar una nota de un estudiante
recibir el id del estudiante,
nota del estudiante = index */
app.delete("/estudiantes/:id/notas/:notaIndex", 
  function (req: Request<notasParams>, res: Response) {
    const idEstudiante = Number(req.params.id);
    const index = Number(req.params.notaIndex);
    ...
    return res
      .status(400)
      .json({ error: "Parametros invalidos" });
  }
  const estudiante = listaEstudiantesFunvaleros.find(e => e.id === idEstudiante);
  if(!estudiante){
    return res.status(404).json({error: "Estudiantes no encontrado"});
  }
  if(index < 0 || index>= estudiante.notas.length){
    return res.status(404).json({error: "Nota no encontrada"});
  }
  const [notaEliminada] = estudiante.notas.splice(index, 1);
